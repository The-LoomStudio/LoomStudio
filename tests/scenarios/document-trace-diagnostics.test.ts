...
describe('Document / Trace / Diagnostics scenarios', () => {
  it('D1: document revision lifecycle preserves history, tombstones deletes, and emits docs.changed', async () => {
  ...
  it('D2: optimistic version conflict rejects stale writes and preserves current document', async () => {
    const { kernel } = createHarness()
    await kernel.start()
    const bridge = createBridge(kernel)
    await bridge.connect()
  ...
  it('D3: ownerExtensionId filtering returns only documents owned by that extension', async () => {
  ...
  it('D4: loom.run trace is visible through trace.list and not stored as a normal document', async () => {
    const { kernel } = createHarness()
    await kernel.start()
    const bridge = createBridge(kernel)
    await bridge.connect()
  ...
  it('D5: diagnostics from loom.run are queryable and emit diagnostics.updated', async () => {
    const { kernel } = createHarness()
    const events: StudioEvent[] = []
    await kernel.start()
