...
import { createErrorResponse, createSuccessResponse, parseRpcRequest, type RpcRequest, type RpcResponse, type StudioEvent } from '@loom-studio/transport'
...
describe('Client Bridge / frontend data-flow scenarios', () => {
  it('C1: client bridge preserves JSON-RPC request source metadata and monotonic request ids', async () => {
  ...
  it('C2: client-only dashboard can read documents, diagnostics, traces, and extension summaries through ClientBridge only', async () => {
  ...
  it('C3: client bridge surfaces missing method errors and remains usable afterward', async () => {
    const { kernel } = createHarness()
    await kernel.start()
    const bridge = createBridge(kernel)
    await bridge.connect()
  ...
  it('C4: disconnect marks the bridge disconnected but does not prevent later calls', async () => {
    const { kernel } = createHarness()
    await kernel.start()
    const bridge = createBridge(kernel)
    await bridge.connect()
  ...
  it('C5: client bridge exposes response metadata for correlation-aware UI diagnostics', async () => {
    const { kernel } = createHarness()
    await kernel.start()
    const bridge = createBridge(kernel)
    await bridge.connect()
