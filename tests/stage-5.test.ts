...
describe('stage 5 mvp acceptance', () => {
  it('runs the headless client end-to-end path across Kernel, Documents, Extension, Loom, Trace, and Diagnostics', async () => {
    ...
    const writeResult = await bridge.call<{ documents: Array<{ id: string; content: { ok: boolean } }> }>('docs.write', {
      id: 'stage-5:doc',
      type: 'example.stage5.note',
      content: { ok: true },
      expectedVersion: 'new',
    })
    const documents = await bridge.call<{ items: Array<{ id: string; content: { ok: boolean } }> }>('docs.list', { type: 'example.stage5.note' })
    const echo = await bridge.call<{ extensionId: string; echo: { message: string } }>('example.echo.echo', { message: 'stage-5' })
    const runResult = await bridge.call<{ fragments: Array<{ content: string }>; traceId?: string }>('loom.run', {
      fragments: [{ id: 'stage-5-fragment', content: 'hello stage 5', meta: { __owner: 'stage-5' } }],
      passes: [{ name: 'uppercase' }],
      trace: { enabled: true },
    })
    await bridge.call('loom.run', {
      fragments: [{ id: 'stage-5-diagnostic', content: 'diagnostic', meta: {} }],
      passes: [{ name: 'missing' }],
    })
    ...
    expect(writeResult.documents[0]).toMatchObject({ id: 'stage-5:doc', content: { ok: true } })
    expect(documents.items[0]).toMatchObject({ id: 'stage-5:doc', content: { ok: true } })
    expect(events.some(event => event.name === 'docs.changed')).toBe(true)
    expect(echo).toEqual({ extensionId: 'example.echo', echo: { message: 'stage-5' } })
    expect(runResult.fragments[0]?.content).toBe('HELLO STAGE 5')
  ...
  it('keeps forbidden runtime/provider/chat fields out of loom.run', async () => {
