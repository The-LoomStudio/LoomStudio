...
> **状态**：独立 Astro 站点已建立 / 部署与后续展示待接续
>
> **日期**：2026-08-27
>
> **触发条件**：准备对外发布 Loom Studio、建立公开文档入口或提供可交互的产品展示。
>
> **事实边界（2026-09-12）**：相邻 `LoomStudio-website` 工程已采用 Astro 静态生成，已有首页、公开文档与独立样式资产；其 README 明确 Cloudflare Pages 部署暂缓。后续网站施工与验证由网站仓库负责，本文保留产品侧分工和未决方向，不声称已上线或全部 Demo 已实现。
...
站点已采用 Astro 静态生成，不再重开框架选型。后续按网站仓库现状核对以下发布能力，不将它们整体视为已完成：
...
5. 现有 Astro 页面是否满足发布范围，是否有真实需求补充文档主题能力；
