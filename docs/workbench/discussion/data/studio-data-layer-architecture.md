...
> **2026-08-12 边界说明**：本文是早期问题域讨论，其中“Kernel-managed state 全部进入 Document Store”“Document 是最小持久化单位”的表述不再是目标架构。当前提案见 [`../../plans/sqlite-data-engine-domain-stores-kernel-plan.md`](../../../archive/plans/sqlite-data-engine-domain-stores-kernel-plan.md)：统一的是 SQLite transaction、Commit Journal 与 Kernel commit event；需要 Revision 的可编辑聚合使用 Document Store，高数量 append-only 业务记录使用 Application-owned Domain Store。
