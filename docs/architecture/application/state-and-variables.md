...
`loom.cardBundle` 当前只接受 `schemaVersion: 2`。导出收集 Card 内联模板与未被内联覆盖的共享模板，并携带 `timelineStateBindings`；导入时相同 ID/version/content 的模板复用，identity 冲突、缺失模板、版本不匹配与 Binding 路径错误会被拒绝。导入仍会物化共享 Definition，因此与工作区已有同 ID 不同内容模板的冲突不会被静默覆盖。
