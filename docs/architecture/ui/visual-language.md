...
- 不给浮动面板添加阴影或发亮外框；
- 不用贯穿容器的边框表达每一级布局；
- 必须分区时使用短分割线：横向两端默认各保留 `8px` 内距，纵向上下默认保留 `16px` 间距；分割线端点仍需与所属内容列的对齐线一致；
- 圆角用于可交互区域和局部表面，不把所有内容包装成卡片；
- 普通按钮的 Hover、Focus 和 Active 优先高亮图标或标题文字，不默认点亮整块背景；需要凸显的按钮可以默认使用强调色图标；
- 列表中的操作项可以在 Hover 和 Focus 时高亮所属行或操作容器背景，以表达其可操作区域；这不作为普通按钮的默认样式；
- 资源工作台只提供 Master Detail 与下钻式两种布局偏好；窄面板自动采用下钻式，宽面板可通过单一布局开关固定为任一模式，不提供 Explorer、Split、Editor 三态切换；
