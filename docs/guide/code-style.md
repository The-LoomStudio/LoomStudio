...
注释只记录无法从命名、类型或局部控制流可靠推断的维护合同。优先说明不变量、协议边界、失败后为何保留状态、兼容路径为何存在，以及真实的性能或容量限制。

不要为字段映射、普通 CRUD、JSX 层级、显而易见的 setter 或逐行翻译代码添加注释。注释不能代替更好的命名、类型、校验或测试；当实现改变时，必须同步删除或更新失效注释。跨包和公共 SDK 接口使用简短英文 TSDoc，内部决策注释也优先使用英文。
