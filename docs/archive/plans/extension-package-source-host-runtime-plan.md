...
> **日期**：2026-08-25
>
> **触发条件**：需要让用户从自定义来源获取 Extension，并在线更新已经安装的 Extension。
>
> **事实边界**：本文记录来源获取、Git 更新、非 Git 替换与 Extension Runtime 的平台边界。用户自行决定来源是否可信；Extension 作者负责自己的数据迁移。现有目录安装、Package / Module / Instance、`activate(ctx)`、desired state 与 capability grant 合同保持不变。
...
用户可以自行指定任意来源，但所有来源必须归一为同一份待验证 Package 输入：
...
NPM Package 不要求发布到 npmjs.com。来源只是用户选择的获取方式，不代表 Loom 对发布者背书。可以包括：
...
```text
来源是 Git
  -> fetch / checkout 用户选择的版本或 commit
  -> 在 staging 校验和构建
  -> 成功后切换 active 版本
...
Loom 负责代码包的获取、安装、替换、激活、失败回滚和权限变化提示，不负责判断来源是否可信，也不负责 Extension 自有数据迁移。Extension 代码目录与 Extension 数据目录必须分离；迁移由 Extension 作者在自己的启动逻辑中负责。
...
当前同进程 Host 无法阻止 Extension 代码直接访问 Node filesystem、network 或 process API。因此，用户信任某个来源不等于 Loom 已经提供恶意代码沙箱。
...
如果未来要把不受信任代码作为公共生态内容推广，才需要额外建设：
...
1. **在线更新核心**：Git 来源拉取与非 Git 包替换，统一进入 staging 和现有 Installer。
2. **版本切换**：保留旧版本、active 指针、启动失败回滚、更新结果展示。
3. **来源记录**：记录用户指定的来源、版本/commit、integrity 和最近一次更新结果。
4. **权限变化提示**：新版本请求新增 capability 时重新询问，不自动继承。
5. **Runtime Isolation**：只有未来明确开放不受信任公共生态时才推进。
6. **依赖求解**：只有真实跨 Extension 依赖出现后才加入。
...
- 替 Extension 作者设计或执行其业务数据迁移；
- 把 Git、Registry 或 Marketplace 固定为唯一发布源；
- 为用户自动判断某个远程来源是否值得信任；
- 更新时静默覆盖 Extension 自有数据；
- 删除 Host、CTX、Grant、Scope 或 lifecycle；
- 把 `ApplicationRuntimeContext` 暴露给普通 Extension。

## 14. 实施前开放问题

1. Git 更新首批支持用户选择 branch、tag 还是 commit；
2. 非 Git 来源首批接受 `.zip`、`.tgz`，还是先复用现有 ZIP 首切片；
3. Extension 代码目录与自有数据目录的默认布局；
4. 更新时是否允许自动构建，还是只接受来源已经提供的可运行产物；
5. 更新失败和新版本启动失败时的回滚边界；
6. Manifest、capability 与版本变化在 UI 中展示到什么粒度。
