...
> **Status**：Completed / 本地官方内容交付完成；远程来源与在线更新已拆分为延期路线
> **日期**：2026-09-11
...
- GitHub Release、NPM、Git URL 或用户自定义地址都只是包来源，不是运行机制。来源解析后进入同一校验与安装路径；用户自行决定来源是否可信，平台不固定发布者，也不能将 `git pull`、`npm install` 或安装脚本直接作用到用户运行工作区。
...
- 新包可用不等于更新当前作者副本。Extension 代码替换由 Extension Source / Host Plan 负责；内容资源替换仍需明确目标、差异/影响与确认，并沿现有版本并发检查。平台不负责 Extension 自有数据迁移。
...
2. 首个远程源的具体形态由用户配置；实施首个更新切片前只需冻结 Git URL、用户自定义 Archive URL、版本/commit 选择和来源记录，不把 GitHub、NPM 或固定可信索引写死为平台合同。
...
3. 远程 Source、安全 Archive 安装、Git 更新、非 Git 版本替换和失败回滚仍未实施；不把本地包导出称为独立在线更新完成。远程来源由用户配置，平台不替用户背书。
...
最终结果：本地官方内容交付完成，本 Plan 归档。官方 starter、显式安装、ST 生命周期解耦、正式导出包与完整 Character Status 样例均已落地，并通过定向 Runtime、Server、Client、扩展构建和 Card Bundle 往返验证。未验证真实在线发布、远程安装、远程更新、浏览器视觉和移动端体验；这些是后续延期路线，不影响本地交付归档。
