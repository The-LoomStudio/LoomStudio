...
- [`application-capability-cli-mcp-adapters-plan.md`](../../workbench/plans/application-capability-cli-mcp-adapters-plan.md)：提供 RPC、Agent Tool、CodeAct、CLI 与 MCP 共用的 Application Capability 执行边界；
- [`file-backed-resource-agent-script-codeact-plan.md`](../../workbench/plans/file-backed-resource-agent-script-codeact-plan.md)：消费本计划形成的执行上下文与 Capability Grant，不另建 Script 权限体系；
...
- Application Capability 首个实际包 / 模块，位置由 [`application-capability-cli-mcp-adapters-plan.md`](../../workbench/plans/application-capability-cli-mcp-adapters-plan.md) 的 Phase 0 决定
...
### WP5：工具执行结果、写入统计与最小 Approval 接缝
...
工具调用前只保留当前工具实际需要的最小允许判断；不在本计划建设通用 RBAC / ABAC 或独立审计平台。工具调用后统一返回可展示的结果，尤其是实际写入的目标、操作、摘要和 Changeset 引用。Session 末尾通过统一写入统计组件展示“修改了什么”，点击后跳转到对应对象或 Changeset。Script / CodeAct 的子调用仍需关联父调用，但不提前开放其直接访问 Store、文件系统或宿主机的能力。

**完成条件**：Workspace Context 不自动等于写入权限；工具在执行前能拒绝越界目标；工具结果统一返回实际写入项；Agent Session 能汇总写入数量、目标类型和跳转引用；已有 Changeset/CAS 结果可追溯。复杂审批策略、永久授权和完整审计查询不属于本工作包。
...
- 统一写入统计所需的 ToolResult / Changeset 引用字段仍需在首个真实写入工具上冻结；不要求先建设独立 Permission UI。
- CodeAct Sandbox / Host 尚未实施；本计划只固定它必须消费的 Capability 边界。
- Workspace Authoring Tool 的完整集合需要依据真实领域 API 分批增加，不能在本计划中预先列举并包装全部操作。

## 最终结果

2026-09-13：WP1 已开始实施。Session 解绑使用 `timelineId: null`，省略字段保持原 Binding；重新绑定到其他 Timeline 明确拒绝。解绑时检查 Session 最新 `run-state`，`running` / `suspended` 拒绝，终态允许。WP2 的核心解析已存在于 `prepareAgentTurn`：Binding 提供 Narrative 默认 Target，Workspace Session 不产生默认 Narrative，绑定 Session 的跨 Timeline Target 被拒绝，PromptBuild 与 Tool Scope 使用同一解析结果。WP4 已提供 `search_prompt_resources`、`read_prompt_resource`、版本化 `update_prompt_resource`，以及 Server RPC 的 Prompt Resource ZIP 导入导出；工具通过 `workspaceResourceAccess` 限制为 Workspace Context，更新复用 Prompt Resource Store 的 CAS、事务和 Changeset。删除被 Agent Profile 引用的预设时，Profile 在同一事务中解除 `presetId` 绑定，后续 Turn 对缺失预设明确失败。WP1/WP2 行为测试 18/18、独立 Workspace Tool Probe 4/4、Workspace RPC、官方内容和 Native Tool Loop 测试通过。正式统一写入结果和 Session 末尾写入统计仍属于 WP5 未完成范围；不再把该工作包描述为大型 Capability Grant、Approval Service 或审计查询平台。
