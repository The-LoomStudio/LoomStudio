...
- Preset / Settings 已有 Prompt Resource Artifact 导入导出，并已补充独立 Prompt Resource ZIP codec、Server RPC、Client API 与工作台入口；完整附件分发和人工验收仍有限制。入口为 `packages/application-runtime/src/runtime/prompt-runtime.ts` 与 `apps/studio-server/src/codecs/prompt-resource-zip.ts`。
- 扩展目录安装、dev-link、嵌套 `dist/` 和包内文件服务已存在；本地 ZIP 安装已接通，安装器拒绝符号链接、越界路径、不安全反斜杠路径及超限包。
...
当前汇总：WP1 完成，WP2 核心目录闭环已实现并通过自动化验证，WP3 已完成 Prompt Resource ZIP 的独立 codec、清单校验、Server RPC、Client API 与资源工作台入口，均待用户交互验收；删除被 Agent Profile 引用的预设时已加入同事务解绑。WP4 已完成 Extension Package ZIP 到现有安全目录安装器、Server RPC、Client API 与工作台上传入口的首个闭环切片，覆盖临时解包、路径越界拒绝、文件数/解压后大小限制与原有 manifest/staging/激活流程；正式发布流程、版本替换策略与用户交互验收仍待接续。完整开发模式与 CLI/MCP 已拆出，不计入本计划未完成项。以下按时间保留历史结果，不以历史阶段描述覆盖当前范围。
...
### 2026-09-13 Extension ZIP 安装首切片

- 新增 `extensions.installPackageZip`；ZIP 先解包至临时目录，拒绝绝对路径、路径穿越与不安全条目，并限制 10000 个文件 / 256 MiB 解压后内容。
- 目录安装器继续负责 manifest、入口文件、staging、安装目录身份和模块激活；解包失败或安装失败均清理临时目录，不执行包内脚本。
- 扩展工作台新增 ZIP 文件选择入口，Client API 与 `extensions.changed` 事件复用现有刷新链路。
- 定向验证：Extension installer、Client API 与 Studio Server RPC 集成共 `21/21` 通过；并覆盖压缩包解压前大小拒绝。Kernel、Studio Server、Client TypeScript 检查通过。完整浏览器点击验收尚未执行。
- WP4 仍保持 In Progress：客户端入口已接通，正式发布来源、版本替换策略和完整安装 UI 验收仍待后续确认。
