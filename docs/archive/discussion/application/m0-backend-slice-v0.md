...
> **Status**: Archived / Superseded M0 Slice  
> **Archived**: 2026-08-28；旧 `Session / NarrativeEntry / submitTurn` 已删除，当前事实见 [`docs/architecture/data/`](../../../architecture/data/) 与 [`docs/architecture/application/agent/`](../../../architecture/application/agent/)。

> **历史说明**：本文只记录已经删除的 M0 实现。下文旧 `Session`、玩家输入写入 Narrative、镜像 `AgentTranscriptEntry` 和 `AgentRuntimeProfile` 均不得视为当前 Schema。
