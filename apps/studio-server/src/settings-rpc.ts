...
import type { NetworkProxyMode, NetworkSettingsStore } from './network-settings.js'
import { createNamespaceRpcCapabilities, type RpcCapability } from './rpc-capability.js'

const settingsRpcMethods = ['settings.network.get', 'settings.network.update'] as const

export function listSettingsRpcCapabilities(): RpcCapability[] {
  return createNamespaceRpcCapabilities({
    names: settingsRpcMethods,
    namespace: 'settings',
    owner: 'studio-server',
